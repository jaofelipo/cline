...
export async function ensureDirExists(...paths: string[]): Promise<string|undefined> 
{
    const dirPath:string = path.join(...paths);
    try 
	{
...
export async function writeFile(filePath: string, content:string, autoCreate:boolean=true)
{
	const fileExists = await fileExistsAtPath(filePath)
	if (!fileExists && autoCreate)
